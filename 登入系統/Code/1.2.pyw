...
def Main0():
    ...
    ttk.Entry(window, textvariable=ver1).place(x=71, y=54)
    # 密碼
    txtid=canvas.create_text(45, 75, anchor="nw")
    canvas.insert(txtid,1,"密碼")
    ttk.Entry(window, show='●', textvariable=ver2).place(x=71, y=74)
    # 按鈕
    ttk.Button(window, text="Sign up", command=Sign_up).place(x=15, y=112)
    ttk.Button(window, text="OK", command=Sign_in).place(x=160, y=112)

    window.mainloop()
def Sign_in():
    global Sign_in_OK, NAME, PASSWORD, name, password
...
def Sign_up():
    ...
    txtid=canvas.create_text(45, 55, anchor="nw")
    canvas.insert(txtid,1,"帳號")
    ttk.Entry(window_sign_up, textvariable=Sign_up_ver1).place(x=71, y=54)
    # 密碼
    txtid=canvas.create_text(45, 75, anchor="nw")
    canvas.insert(txtid,1,"密碼")
    ttk.Entry(window_sign_up, textvariable=Sign_up_ver2).place(x=71, y=74)
    # 確認密碼
    txtid=canvas.create_text(21, 95, anchor="nw")
    canvas.insert(txtid,1,"確認密碼")
    ttk.Entry(window_sign_up, textvariable=Sign_up_ver3).place(x=71, y=94)
    # 按鈕
...
def Main1():
    ...
    canvas.insert(txtid,1,NAME)
    # 按鈕
    ttk.Button(window, text="Sign up", command=Setup).place(x=15, y=112)
    ttk.Button(window, text="OK").place(x=160, y=112)
